...
import { CategoryData } from "../TableCategories/table";
...
interface TableDefaultProps {
  ...
  onModalChange: (modal: boolean, ProductData?: any) => void;
  onDialogChange: (deletebtn: boolean, ProductData?: any, CategoryData?:any) => void;
}
...
export default function TableDefault(props: TableDefaultProps) {
  ...
  const { data, categories, onModalChange, onDialogChange } = props; // Data
  const [, setEditButton] = useState(false); // Edit button
  const [, setDeleteButton] = useState(false); // Delete button

  const handleEditButton = (modal: boolean, ProductData?: ProductData) => {
    onModalChange(modal, ProductData);
    setEditButton(modal);
  };
  ...
  const handleDeleteOpen = (
    ...
    onDialogChange(deletebtn, ProductData, categories.find(category => category.id === ProductData.category_id));
          ...
          {(rowsPerPage > 0
            ...
            <TableRow key={row.id}>
              ...
              <TableCell
                ...
                align="center"
              >
                {categories.find(category => category.id === row.category_id)?.description || "Categoria não encontrada"}
              </TableCell>
              <TableCell
                component="th"
                scope="row"
